...
func TestServer(t *testing.T) {
	...
	go func() {
		...
		client := NewClient(jsonrpc2.HeaderFramer(), heartbeatInterval)
		if err := client.Run(ctx, w, r); err != nil {
			errProducer = err
		}
	}()
	go func() {
		defer wg.Done()
		server := NewServer(jsonrpc2.HeaderFramer())
		if err := server.Run(ctx, r, w); err != nil {
			errConsumer = err
		}
	}()
